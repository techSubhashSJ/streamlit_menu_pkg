...
class Register:
    ...
    def init(self):
        if not hasattr(_components.register_widget, "__callbacks_patched__"):
            ...
            _components.register_widget = self._patch_register_widget(
                _components.register_widget)

    def _patch_register_widget(self, register_widget):
    ...
    def register_callback(self, element_key, callback, *callback_args,
                          **callback_kwargs):
    ...
    def get_component_rerender_count(self, element_key):
    ...
    def set_component_rerender_count(self, element_key):
        # This funtion will record the rerendering count
        _state["_component_render_count"][
            element_key] = self.get_component_rerender_count(element_key) + 1
        return _state["_component_render_count"][element_key]
